...
frm.addEventListener("submit", (e) => {
    ...
    const dtVencimento = frm.inVencimento.value
    ...
    const parte = dtVencimento.split("-") // data vem no formato aaaa-mm-dd

    vencimento.setDate(Number(parte[2]))
    vencimento.setMonth(Number(parte[1])-1)
    vencimento.setFullYear(Number(parte[0]))
    // calcular a diferença enre datas (em milissegundos)
